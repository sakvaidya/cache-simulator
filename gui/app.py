...
from gui.config_panel import ConfigPanel, TaskPanel, ClockPanel
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.clock_panel = ClockPanel(self.left_frame,
    ...
    def _step_once(self):
        ...
        result, si, bi = self.cache.access(address, self.task_name)
    ...
    def _on_reset(self):
        ...
        self.ref_index = 0
        if self._cfg:
            self.cache_view.setup(
                self._cfg["num_sets"],
                self._cfg["cache_size"] // self._cfg["num_sets"]
            )
        self.ref_bar.set_refs(self.memory_refs)
        self.stats_panel.update_from_cache(self.cache) if self.cache else None
